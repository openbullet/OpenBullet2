...
public class BlockSolveCaptcha : BlockBase
{
    ...
    public override async Task Process(LSGlobals ls)
    {
        ...
        try
        {
            try
            {
                var response = await GetResponseAsync(ls, sessionParams);

                InsertVariable(ls, false, response.Id, "CAPTCHAID");
    ...
    private async Task<CaptchaResponse> GetResponseAsync(LSGlobals ls, SessionParams sessionParams)
    {
        ...
        return Type switch
        {
            ...
            CaptchaType.ReCaptchaV2 => await provider.SolveRecaptchaV2Async(
                ReplaceValues(SiteKey, ls), ReplaceValues(SiteUrl, ls), "", false, IsInvisible, sessionParams),

            CaptchaType.ReCaptchaV3 => await provider.SolveRecaptchaV3Async(
                ReplaceValues(SiteKey, ls), ReplaceValues(SiteUrl, ls),
                ReplaceValues(Action, ls), float.Parse(ReplaceValues(MinScore, ls)), false, sessionParams),

            CaptchaType.FunCaptcha => await provider.SolveFuncaptchaAsync(
                ReplaceValues(PublicKey, ls), ReplaceValues(ServiceUrl, ls),
                ReplaceValues(SiteUrl, ls), NoJS, data: null, sessionParams),

            CaptchaType.HCaptcha => await provider.SolveHCaptchaAsync(
                ReplaceValues(SiteKey, ls), ReplaceValues(SiteUrl, ls),
                invisible: false, enterprisePayload: null, sessionParams),

            CaptchaType.Capy => await provider.SolveCapyAsync(
                ReplaceValues(SiteKey, ls), ReplaceValues(SiteUrl, ls), sessionParams),
            ...
            CaptchaType.GeeTest => await provider.SolveGeeTestAsync(
                ReplaceValues(GT, ls), ReplaceValues(Challenge, ls),
                ReplaceValues(ApiServer, ls), ReplaceValues(SiteUrl, ls), sessionParams),

            _ => throw new NotSupportedException(
                $"The currently selected service ({provider.ServiceType}) does not support tasks of type {Type}")
        };
    }
}
