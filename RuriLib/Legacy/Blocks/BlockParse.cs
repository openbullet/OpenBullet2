...
public class BlockParse : BlockBase
{
    ...
    public bool DotMatches { get; set; }
    ...
    public bool JTokenParsing { get; set; }
    #endregion
    ...
    public string RegexOutput { get; set; } = string.Empty;
    #endregion
    ...
    public override async Task Process(LSGlobals ls)
    {
        ...
        {
            if (CssElementIndex < 0 || CssElementIndex >= list.Count)
            {
                throw new ArgumentOutOfRangeException(nameof(CssElementIndex), "The CSS element index is out of range");
            }

            list = [list[CssElementIndex]];
        }
    ...
    private static List<string> ParseJSON(string original, string fieldName, bool jTokenParsing)
    {
        ...
        ParseJSON(string.Empty, original, jsonlist);
    ...
    private static void ParseJSON(string a, string b, List<KeyValuePair<string, string>> jsonlist)
    {
        ...
        if (b.StartsWith("["))
        {
            ...
            {
                ParseJSON(string.Empty, i.ToString(), jsonlist);
            }
        }
        ...
        if (b.Contains("{"))
        {
            ...
            {
                ParseJSON(o.Key, o.Value?.ToString() ?? string.Empty, jsonlist);
            }
        }
    }
}
