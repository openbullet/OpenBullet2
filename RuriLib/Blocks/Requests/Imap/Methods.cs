...
public static class Methods
{
        ...
        ImapAutoConnectMode mode = ImapAutoConnectMode.Full)
    {
        ...
        {
            data.Logger.Log($"Failed to query {thunderbirdUrl}: {ex.GetType().Name}: {ex.Message}", LogColors.DarkOrchid);
        }
        ...
        {
            data.Logger.Log($"Failed to query the MX records: {ex.GetType().Name}: {ex.Message}", LogColors.DarkOrchid);
        }
    ...
    private static async Task<bool> TryConnect(BotData data, ImapClient client, string domain, HostEntry entry)
    {
        ...
        {
            data.Logger.Log($"Failed! {ex.GetType().Name}: {ex.Message}", LogColors.DarkOrchid);
        }
