...
public static class Methods
{
        ...
        SmtpAutoConnectMode mode = SmtpAutoConnectMode.Full)
    {
        ...
        {
            data.Logger.Log($"Failed to query {thunderbirdUrl}: {ex.GetType().Name}: {ex.Message}", LogColors.LightBrown);
        }
        ...
        {
            data.Logger.Log($"Failed to query the MX records: {ex.GetType().Name}: {ex.Message}", LogColors.LightBrown);
        }
    ...
    private static async Task<bool> TryConnect(BotData data, SmtpClient client, string domain, HostEntry entry)
    {
        ...
        {
            data.Logger.Log($"Failed! {ex.GetType().Name}: {ex.Message}", LogColors.LightBrown);
        }
    ...
    private static IProxyClient MapProxyClient(Proxy proxy)
    {
        if (!proxy.NeedsAuthentication)
        {
            return proxy.Type switch
            {
                ...
                ProxyType.Https => throw new BlockExecutionException("HTTPS proxies are not supported for SMTP requests"),
