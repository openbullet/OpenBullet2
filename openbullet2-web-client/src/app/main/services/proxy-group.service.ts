...
export class ProxyGroupService {
    ...
    getAllProxyGroups() {
        return this.http.get<ProxyGroupDto[]>(
            getBaseUrl() + '/proxy-group/all'
        );
    }

    createProxyGroup(guest: CreateProxyGroupDto) {
        return this.http.post<ProxyGroupDto>(
            getBaseUrl() + '/proxy-group', guest
        );
    }

    updateProxyGroup(updated: UpdateProxyGroupDto) {
        return this.http.put<ProxyGroupDto>(
            getBaseUrl() + '/proxy-group', updated
        );
    }

    deleteProxyGroup(id: number) {
        return this.http.delete(
            getBaseUrl() + '/proxy-group',
            {
                params: {
                    id
                }
